    ...
    int benchmark, int benchmark_layers)
{
    ...
    //BND
    int person_cnt = 0;
    thread_mgm_t *sending = THREAD_MGM_INIT();


    sending->start[0] = capture_start;
    sending->start[1] = xfer_start;

    pthread_cond_init(&detect_cond, NULL);
    pthread_mutex_init(&detect_mutex, NULL);


    for(int tl = 0; tl < 2; tl++) {
      if(THREAD_CREATE(&sending->tid[tl], NULL, sending->start[tl], (void *)sending) < 0) {
...
#if 0 
            if((person_cnt = get_person_detect()) > 0) {
              printf("%d person detected \n", person_cnt);
            }
            else {
              printf("can not find the person (%d)\n", person_cnt);
            }
#endif
            
            person_cnt = get_person_detect();
