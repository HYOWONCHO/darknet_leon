...
static int person_detect = 0L;

extern "C" int get_person_detect(void) {
  return person_detect;
}

extern "C" void release_person_detect(void) {
  person_detect = 0;
}
...
extern "C" void draw_detections_cv_v3(mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output)
{
    try {
        ...
        //printf("[%s:%d - BND info] : Detected Num : %d \n", __func__, __LINE__,num);

        for (i = 0; i < num; ++i) {
            char labelstr[4096] = { 0 };
            int class_id = -1;
            for (j = 0; j < classes; ++j) {
              if(!strncmp(names[j], "person", 5) && dets[i].prob[j] * 100 >= 50) {
                  person_detect += 1;
                  //printf("[%s:%d - BND info] : Detected Names : %s \n", __func__, __LINE__, names[j]);
